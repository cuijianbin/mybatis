...
import com.mybatis.generator.tool.ConfigUtil;
import com.mybatis.generator.tool.DataTypeUtil;
import com.mybatis.generator.tool.FileUtil;
import com.mybatis.generator.tool.JdbcTypeUtil;
import com.mybatis.generator.tool.NameUtil;
import com.mybatis.generator.tool.ParamTypeUtil;
...
public class MapperXmlAutoDaoImpl implements MapperXmlAutoDao {
	...
	public boolean createMapperXml() {
		...
		if ("true".equals(mapperXmlFalg)) {
			...
			// Mapper.xml的路径
			String path = projectPath + "/src/main/java/" + mapperXmlPath;
			// 遍历装有所有表结构的List
			for (int i = 0; i < list.size(); i++) {
				...
				String beanName = NameUtil.fileName(tableName) + "Entity";
				...
				insertRecordCon.append(
						"\t" + "<insert id=\"insertRecord\" parameterType=\"" + beanPackage + "." + beanName + "\">\n");
				...
				insertSelCon.append("\t" + "<insert id=\"insertSelective\" parameterType=\"" + beanPackage + "."
						+ beanName + "\">\n");
				...
				delByIdCon.append("\t\t" + "delete from " + tableName + " where " + IdName + "= #{" + beanIdName
						+ ",jdbcType=" + IdJdbcType + "}\n");
				delByIdCon.append("\t" + "</delete>\n");

				StringBuffer updateByIdSelCon = new StringBuffer();
				updateByIdSelCon.append("\t" + "<update id=\"updateByIdSelective\" parameterType=\"" + beanPackage + "."
						+ beanName + "\">\n");
				updateByIdSelCon.append("\t\t" + "update " + tableName + "\n" + "\t\t" + "<set>\n");

				StringBuffer updateByIdCon = new StringBuffer();
				updateByIdCon.append(
						"\t" + "<update id=\"updateById\" parameterType=\"" + beanPackage + "." + beanName + "\">\n");
				...
				countSelCon.append("\t" + "<select id=\"countSelective\" parameterType=\"" + beanPackage + "."
						+ beanName + "\" resultType=\"java.lang.Integer\">\n");
				...
				selectByIdCon.append("\t" + "<select id=\"selectById\" parameterType=\"" + IdParamType
				...
				for (int j = 0; j < columns.size(); j++) {
					...
					if (j == 0) {
						resultMapCon.append("\t\t" + "<id column=\"" + columnName + "\" property=\"" + attrName
								+ "\" jdbcType=\"" + jdbcType + "\"/>\n");
						baseColCon.append("\t\t" + columnName);
						insertRecordCon.append(columnName);
						insertRecordCons.append("#{" + attrName + ",jdbcType=" + jdbcType + "}");
					} else {
						resultMapCon.append("\t\t" + "<result column=\"" + columnName + "\" property=\"" + attrName
								+ "\" jdbcType=\"" + jdbcType + "\"/>\n");
						baseColCon.append("," + columnName);
						insertRecordCon.append(",\n" + "\t\t\t" + columnName);
